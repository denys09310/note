...
class Widget(QMainWindow):
    def   __init__(self):
        ...
        self.ui.createteg_btn.clicked.connect(self.create_note)
        self.ui.deleteteg_btn.clicked.connect(self.del_tag)
    ...
    def save_note(self):
        tags = []
        for i in range(self.ui.list_wid.count()):
            tags.append(self.ui.list_wid.item(i).text())
    ...
    def add_tag(self):
        teg_name = self.ui.list_wid.text()
        if tag_name!="":
            if tag_name in self.notes[self.name]['tag']:
                self.notes[self.name]['tag'].append(tag_name)
                self.ui.list_wid.clear()
                self.ui.list_wid.addItems(self.notes[self.name]['tag'])

    def del_tag(self):
        if self.ui.list_wid.selectedItems()[0].text():
            tag_name = self.ui.list_wid.selectedItems()[0].text()
            if tag_name in self.notes[self.name]['tag']:
                self.notes[self.name]['tag'].remove(tag_name)
                self.ui.list_wid.clear()
                self.ui.list_wid.addItems(self.notes[self.name]['tag'])
